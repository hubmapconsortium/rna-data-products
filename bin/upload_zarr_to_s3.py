...
def upload_dir_to_s3(local_dir: Path, uuid: str):
    ...
    for root,dirs,files in os.walk(local_dir, topdown=False):
        for local_file in files:
            ...
            os.system(f"aws s3 cp \"{file_name}\" \"{bucket_path}{dir_name}/{file_name}\" --expected-size \"{file_size}\"")
...
def main(zarr_dir: Path, metadata_file: Path, access_key_id: str, secret_access_key: str):
    ...
    zarr_file = shutil.copytree(f"{zarr_dir}/secondary_analysis.zarr","./zarr/secondary_analysis.zarr")
